...
public class StringTools {
    ...
    public static String getVerificationKey(String email, Integer type) {
        EmailCodeTypeEnum emailCodeTypeEnum = EmailCodeTypeEnum.getByType(type);
        if (isEmpty(email) || emailCodeTypeEnum == null) {
            return null;
        }
        return String.format("%s:%s:%s:%s", Constants.REDIS_KEY_VERIFICATION_CODE, emailCodeTypeEnum.name().toLowerCase(), email, UUID.randomUUID().toString());
    }

    public static String digestMd5(String originString) {
