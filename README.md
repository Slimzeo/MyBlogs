...
- 首页、Blogs 文章正文、Notes 笔记、普通页面、搜索、分类、标签、归档、友链和评论均可公开访问，无需登录。
- `/admin/**` 后台管理需要登录。
...
- 首页、已发布文章、Notes 笔记、页面、搜索、分类、标签、归档、友链和公开评论：不需要登录。
...
2. 点击右上角 `写文章`。
3. 填写标题。
4. 选择内容格式并填写正文：普通文章使用 Markdown；完整网页可选择 HTML，并从本地载入 `.html` 文件。
...
7. 状态选择：
   ...
   - `私密`：不会出现在首页、搜索、归档、分类或标签列表；只有登录后台的管理员
     通过文章地址才能查看。
...
文章的“显示时间”可在后台自定义，首页排序、搜索/分类列表和归档月份都以它为准；
“实际创建时间”只在首次保存时生成，之后不会被编辑。显示时间留空时等于实际创建时间。
...
顶部的 `Blogs` 进入普通博客，保留分类、标签和时间归档；顶部的 `Notes` 进入仓库里的
`notes/` Markdown Wiki。Notes 会按文件夹结构生成 Explorer，文件夹页面展示子目录和
Markdown 文件，点击文件进入笔记正文。`/topics` 是 Blogs 内部的文章目录入口。

博客文章仍然保留原有的分类和标签筛选能力；森林背景与首页落叶动画只属于公共主题，
不会改变 Blogs 的文章数据结构。Notes 的路径可通过 `BLOG_NOTES_DIR` 配置，默认是 `notes`。
...
- 访客从前台导航栏导入正确密钥后，可在当前浏览器和当前访问 IP 下查看 24 小时；
- Cookie 被复制到其他 IP，或设备切换网络后，授权立即失效；
