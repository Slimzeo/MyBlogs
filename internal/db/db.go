...
func seed(gdb *gorm.DB, cfg *config.Config) error {
	...
	if optCount == 0 {
		opts := []model.Option{
			...
			{Name: "module_notes_enabled", Value: "1"},
			{Name: "module_archives_enabled", Value: "1"},
			{Name: "module_links_enabled", Value: "1"},
			{Name: "module_about_enabled", Value: "1"},
			{Name: "about_motto", Value: "努力看论文，学习看源码，成为 Agent 大王"},
			{Name: "social_github", Value: "https://github.com/Slimzeo"},
